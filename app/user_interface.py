...
class UserInterface:
    ...
    def __init__(self):
        ...
        self.start_msg = 'Welcome to Wut R Rox!\n'
        ...
        self.option_add_attribute = 'add an attribute'
        self.option_edit_attribute = 'edit an attribute'
        self.option_remove_attribute = 'remove an attribute'
        self.option_display_all_chars = 'display all characteristics of matched minerals'
        self.option_identify_new_mineral = 'start identifying a different mineral'
        self.options = [self.option_add_attribute, self.option_edit_attribute,
                        self.option_remove_attribute, self.option_display_all_chars,
                        self.option_identify_new_mineral, self.quit_option]

    def print_start_msg(self):
        print(self.start_msg)

    def get_input(self, message, selection_options):
        while True:
            print(message)
            for option in range(len(selection_options)):
                print('{}) {}'.format(str(option), str(selection_options[option])))
            try:
    ...
    def get_sampletype(self):
        return self.get_input(self.sample_type_msg, self.sample_type_options)

    def get_different_input(self, function_to_restart):
